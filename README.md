...
- **사료/신뢰도 UI** — 사건별 출처 각주(`src/data/sources.ts`), ⚠ 근사 좌표 "위치 추정" 배지
- **인물 카드** — 심일·조달진·김종오 등 약력(`src/data/people.ts`), 상세 패널 인물 칩 → 카드
- **한반도 인셋 미니맵** — 38선·서울(6/28 함락 표시)·현재 뷰박스(팬/줌 추종)
- **연속 시간 스크러버** — 슬라이더 드래그로 전선 날짜 간 연속 보간
- **전선 남하 속도 그래프** — 일자별 남하 km/일, "3일 지연→급가속" 서사 시각화
- **온보딩 코치마크**(3단계·1회), **PWA 오프라인**(manifest + service worker)
- **KO/EN 다국어** — UI 크롬 번역(`src/i18n/`), 긴 서술은 KO 유지(번역 보류)
- 키보드(←/→ 날짜·Esc 닫기·Space 브리핑) + 패널 스와이프, 그리스펜슬 손떨림 필터
